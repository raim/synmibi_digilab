...
files <- list.files(path=in.path, pattern="measurement_0.*\\.TXT$")
...
sampleLabels <- sub("^SAMPLE3_","",sampleIDs)

sampleRuns <- sub("[0-9]*","", sampleLabels)
...
for ( i in 1:length(RUNS) ) {
    ...
    cnts <- counts[,rid,drop=FALSE]
    cnts.nrm <- counts.nrm[,rid,drop=FALSE]
    cnts.all <- counts.all[,rid,drop=FALSE]
